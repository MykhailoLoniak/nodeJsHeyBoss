...
const getToken = async (req, res) => {
  ...
  const redirectUrl = `${process.env.CLIENT_ORIGIN}/auth?firstName=${encodeURIComponent(user.first_name)}&lastName=${encodeURIComponent(user.last_name)}&userId=${encodeURIComponent(user.id)}&role=${encodeURIComponent(user.role)}&accessToken=${encodeURIComponent(tokens.accessToken)}`;
...
function dataUrl(url) {
  if (!url) url = "/uploads/avatars/defaultAvatar.svg"
